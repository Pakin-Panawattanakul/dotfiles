...
```sh
sudo nixos-rebuild switch --flake ~/'dotfiles/nixos?submodules=1#'nixos-T480
```
...
**Do NOT run `./update` on the NixOS system.** Configs are deployed via `sudo nixos-rebuild switch` → home-manager → `nixos/home-manager/config-symlink.nix`, which creates out-of-store **symlinks** from `config/` and `files/` into `~/.config` and `~/`. `./update` (GNU stow + dwl/someblocks builds) lives in `archives/` and is a leftover from the non-NixOS (Void) setup.

## Architecture

- **NixOS flake** is in `nixos/` (`nixos/flake.nix`) and declares hosts `nixos-T480`, `nixos-home`, `nixos-NV15` using nixpkgs stable (`nixos-26.05`) + home-manager `release-26.05`. `slang-server` (not packaged on 26.05) is tracked as a pinned `git+…?submodules=1` input (vendored deps), threaded via `specialArgs` and built into `pkgs` through an overlay in `nixos/configuration.nix` (source of truth: `nixos/pkgs/slang-server.nix`). ncspot comes from stable nixpkgs (v1.4.0, includes the login-API fix).
- **Per-host wiring** lives in `nixos/flake.nix`: each `nixosSystem` imports `./hosts/hardware-configuration-<host>.nix`, `./configuration.nix`, its own `modules/` set, and home-manager. Wiring today:
...
- **home-manager** (`nixos/home.nix` = user `pakin`) imports modules from `nixos/home-manager/`:
  ...
  - plus `neovim.nix`, `terminal.nix` (foot/zsh/oh-my-zsh/tmux/starship), `firefox.nix` (reads `files/Templates/user.js` + `nixos/icons/*.svg`), `wayland.nix` (wlroots utils + wmenu scripts from `wmenu-scripts`), `music.nix`, `desktop-utils.nix`, `books-library.nix`. `rclone-gdrive.nix` and `synology-drive.nix` are present but commented out in `home.nix`.
- **dwl** + **someblocks** are git submodules under `build/`. `nixos/modules/dwl.nix` builds dwl from the submodule source, overriding `config.h`, and builds someblocks with `config/.config/dwl/blocks.h` injected (no `./update` needed).
- **opencode**: installed via home-manager (`nixos/home-manager/terminal.nix`), and its config `config/.config/opencode/opencode.jsonc` **is** symlinked by `nixos/home-manager/config-symlink.nix` — it is nix/managed. Repo-local `.opencode/` (skills, plugins) is separate from the user config.
- `nixos/hosts/*.nix` hardware configs are auto-generated — do not edit (add to `nixos/configuration.nix` instead).
...
| What | Where |
|---|---|
| NixOS flake / lockfile / per-host wiring | `nixos/flake.nix`, `nixos/flake.lock` |
| NixOS system config (shared) | `nixos/configuration.nix` |
| Per-host hardware configs (generated, don't edit) | `nixos/hosts/hardware-configuration-<host>.nix` |
| System modules | `nixos/modules/` (battery, dwl, kanata, nvidia, udev, wifi) |
| Home-manager user config + modules | `nixos/home.nix` + `nixos/home-manager/` |
...
| Shell dotfiles (sourced by `nixos/home-manager/terminal.nix`) | `config/.zshrc`, `.zshenv`, `.zprofile`, `.profile`, `.bashrc`, `.tmux.conf` |
| Symlinked home dirs / files (Scripts, Templates, wallpapers) | `files/` |
| Launcher icons (used by `nixos/home-manager/firefox.nix`) | `nixos/icons/` |
| Buildable submodules (dwl, someblocks, bgutil-ytdlp-pot-provider) | `build/` |
| Wmenu scripts submodule | `submodules/wmenu-scripts/` |
...
- dwl config lives at `config/.config/dwl/` (`config.h`, `blocks.h`, `autostart`, `start-dwl`), consumed by `nixos/modules/dwl.nix`.
- `.zshrc` has a non-NixOS fallback block (oh-my-zsh bootstrap) plus aliases for `rebuild`, `nixgc`, spotdl/`music-sync`, and the pot-provider (radicale/ytdlp) that `cd`s into `build/bgutil-ytdlp-pot-provider`.
